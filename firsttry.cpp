...
int main(int argc, char *argv[]) {
    ...
    for (const auto& inputFileName : inputFiles) {
        ...
        cout << "Untagged Events (triggerBits = 2, not muon/Michel, PMT activity): " << num_untagged << "\n";
        cout << "------------------------\n";
        f->Close();
        num_events = 0;
        num_muons = 0;
    ...
    h_pmt_energy_cosmic->SetLineWidth(3);
    h_pmt_energy_untagged->SetLineWidth(3);
    h_pmt_energy_tagged->SetLineWidth(2);
    h_pmt_energy_veto_pass->SetLineWidth(2);
    h_pmt_energy_untagged->SetLineStyle(7);
    h_pmt_energy_tagged->SetLineStyle(2);
    h_pmt_energy_cosmic->SetTitle("PMT Energy by Category;Energy (p.e.);Events");
    double max_events_categories = h_pmt_energy_untagged->GetMaximum();
    if (max_events_categories == 0) {
        max_events_categories = std::max({h_pmt_energy_cosmic->GetMaximum(),
    ...
    h_pmt_energy_untagged->Draw("HIST");
    h_pmt_energy_cosmic->Draw("SAME HIST");
    h_pmt_energy_tagged->Draw("SAME HIST");
    h_pmt_energy_veto_pass->Draw("SAME HIST");
    gPad->SetLogy(1);
    TLegend* leg_categories = new TLegend(0.15, 0.7, 0.35, 0.9);
